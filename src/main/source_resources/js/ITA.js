...
document.addEventListener("DOMContentLoaded", function(event) {
    var elm = jQuery('[ita-messenger]');
    var crm_path = (elm[0].getAttribute('path'));
    var elm = jQuery('[ita-messenger]').load(crm_path + "/static_templates/itaMessegger.html", function() {
        var busy = false;
        var state = 1;
        ...
        jQuery("iframe").on('load', function() {
            ...
            setTimeout(function() { res_elem.removeClass("disable-animation"); }, 300);
            jQuery(".chat").draggable({ cancel: ".ignore", containment: "parent", start: dragstart, stop: dragend });
            reinitElemPos();
        });
    });
});
