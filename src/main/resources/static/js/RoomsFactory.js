...
springChatServices.factory('RoomsFactory', ['$injector', '$route', '$routeParams', '$http', '$location', '$interval', '$cookies', '$timeout', 'toaster', 'ChatSocket', '$cookieStore', '$q', '$sce', '$rootScope', 'ChannelFactory', function ($injector, $route, $routeParams, $http, $location, $interval, $cookies, $timeout, toaster, chatSocket, $cookieStore, $q, $sce, $rootScope, ChannelFactory) {
    ...
    var addRoomWithBot = function addRoomWithBot(roomName) {
    ...
    var findParticipant = function findParticipant(nickname) {
        for (var c_index in participants) {
            if (participants[c_index].username == nickname) return participants[c_index];
        }return null;
    };
    var unsubscribeWatch = null;

    var goToRoom = function goToRoom(roomId) {
        ...
        if (currentRoom !== undefined && getRoomById(rooms, currentRoom) !== undefined) getRoomById(rooms, currentRoom.roomId).date = curentDateInJavaFromat();
        var evn = goToRoomEvn(roomId);
        return evn;
    };

    var goToRoomById = function goToRoomById(roomId) {
    ...
    function calcPositionUnshift(msg) {
        ...
        if (summarised == false) messages.unshift(msg);
    ...
    function calcPositionPush(msg) {
        ...
        if (messages.length > 0) {
            if (messages[messages.length - 1].username == msg.username) msg.position = messages[messages.length - 1].position;else msg.position = !messages[messages.length - 1].position;
        } else msg.position = false;
        ...
        $rootScope.$$postDigest(function () {
            var objDiv = document.getElementById("messagesScroll");
            if (needScrollDown) objDiv.scrollTop = 99999999999; //objDiv.scrollHeight;
        });
    }
    ...
    var changeRoom = function changeRoom() {
        ...
        if (ChannelFactory.isSocketSupport() === true) {
            lastRoomBindings.push(chatSocket.subscribe("/topic/{0}/chat.message".format(currentRoom.roomId), function (message) {
                calcPositionPush(JSON.parse(message.body)); //POP
            }));
            lastRoomBindings.push(chatSocket.subscribe("/topic/chat/rooms/{0}/remove_user/{1}".format(currentRoom.roomId, UserFactory.getChatUserId()), function (message) {
                if (!UserFactory.isAdmin()) unsubscribeCurrentRoom();
            }));
        ...
        lastRoomBindings.push(chatSocket.subscribe("/topic/{0}/chat.typing".format(currentRoom.roomId), function (message) {
    ...
    var addTenantToRoom = function addTenantToRoom(id) {
        $http.post(serverPrefix + "/bot_operations/tenant/{0}/askToAddToRoom/{1}".format(id, currentRoom.roomId), {}).success(function (data, status, headers, config) {
    ...
    var addUserToRoom = function addUserToRoom(chatUserId) {
        $http.post(serverPrefix + "/chat/rooms.{0}/user/add?chatId={1}".format(currentRoom.roomId, chatUserId), {}).success(function (data, status, headers, config) {}).error(function (data, status, headers, config) {});
    };

    var removeUserFromRoom = function removeUserFromRoom(userId) {
        $http.post(serverPrefix + "/chat/rooms.{0}/user.remove/{1}".format(currentRoom.roomId, userId), {}).success(function (data, status, headers, config) {
            if (data == false) {
     ...
     * UPDATE MESSAGE LP
     *************************************/
    var subscribeMessageLP = function subscribeMessageLP() {
        var currentUrl = serverPrefix + "/{0}/chat/message/update".format(currentRoom.roomId);
        ajaxRequestsForRoomLP.push($.ajax({
            type: "POST",
    ...
    var subscribeParticipantsLP = function subscribeParticipantsLP() {
        ...
        ajaxRequestsForRoomLP.push($.ajax({
            ...
            success: function success(data) {
                subscribeParticipantsLP();
                var parsedData = JSON.parse(data);
                if (parsedData.hasOwnProperty("participants")) participants = parsedData["participants"];
            },
            error: function error(xhr, text_status, error_thrown) {
    ...
    function loadMessagesContains(searchQuery) {
        ...
        $http.post(serverPrefix + "/chat/room/{0}/get_messages_contains".format(currentRoom.roomId), searchQuery).success(function (data, status, headers, config) {
    ...
    function loadSubscribeAndMessageLP() {
        $http.post(serverPrefix + "/{0}/chat/participants_and_messages".format(currentRoom.roomId), {}).success(function (data, status, headers, config) {
            loadSubscribeAndMessage(data);
        }).error(function (data, status, headers, config) {});
    }

    var checkUserAdditionPermission = function checkUserAdditionPermission(chatUserId) {
        if (currentRoom == undefined) return false;
    ...
    var checkMessageAdditionPermission = function checkMessageAdditionPermission() {
        if (currentRoom == undefined) return false;
        if (typeof currentRoom === "undefined") return false;
    ...
    var unsubscribeCurrentRoom = function unsubscribeCurrentRoom(event) {
    ...
    function afterIsInited() {
        ...
        if (ChannelFactory.getIsInited() == true) {
            ...
            if (room != null && isRoomConsultation(room)) //redirect to consultation
                {
                    $http.post(serverPrefix + "/chat/consultation/fromRoom/" + room.roomId).success(function (data, status, headers, config) {
                        if (data == "" || data == undefined) $rootScope.goToAuthorize(); //not found => go out
                        else $location.path("consultation_view/" + data);
            ...
            $http.post(serverPrefix + "/bot_operations/tenant/did_am_wait_tenant/{0}".format(currentRoom.roomId)).success(function (data, status, headers, config) {
                if (data == true) showToasterWaitFreeTenant();
            }).error(function (data, status, headers, config) {
    ...
    var updateRooms = function updateRooms(message) {
        var parseObj;
        if (ChannelFactory.isSocketSupport() === true) {
    ...
    function subscribeRoomsUpdateLP() {
        ...
        $http.post(serverPrefix + "/chat/rooms/user/login").success(function (data, status, headers, config) {
    ...
    var goToPrivateDialogErr = function goToPrivateDialogErr(data, status, headers, config) {
    ...
    function goToPrivateDialog(intitaUserId) {
        $rootScope.hideMenu();
        $http.post(serverPrefix + "/chat/rooms/private/" + intitaUserId).success(function (data, status, headers, config) {
            console.log("PRIVATE ROOM CREATE OK ");

            //$scope.goToDialogById(data);
            if (RoomsFactory.getCurrentRoom() == null) currentRoom = {};
    ...
    var newMsgNumber = 0;
    var updateNewMsgNumber = function updateNewMsgNumber(diff) {
        if (diff != undefined) {
            newMsgNumber += diff;
            return;
        }
    ...
    var RoomsFactory = {
        ...
        setRooms: function setRooms(roomsArg) {
            rooms = roomsArg;
            updateNewMsgNumber();
        },
        ...
        getNewMsgNumber: function getNewMsgNumber() {
            return newMsgNumber;
        },
