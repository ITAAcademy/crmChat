...
var BOT_ELEMENTS_MODULE = function () {
    ...
    var BotElementPropertiesTypes = {
        "botcalendar": { "itemvalue": "date" },
        "bottext": { "textcolor": "color" },
        "botrating": { "starcolor": "color" }
    };
    publicData.getBotElementPropertyType = function (botelement, propertyName) {
        ...
        if (typeof botElementType != 'undefined') {
            if (botElementType.hasOwnProperty(propertyName)) return botElementType[propertyName];
        }
    ...
    publicData.isContainer = function (type) {
        if (type == "bot-container" || 'bot-list' == type) return true;
        return false;
    };
        ...
        object.getHTML = function (scope, ignoreAddedProperties) {
            ...
            for (var key in this.properties) {
                ...
                if (key == "content") {
                    ...
                    var escapedValue;
                    if (typeof value === "string") escapedValue = value.escapeHtml();else if (getType(value) == "array") escapedValue = JSON.stringify(value).escapeHtml();else if (getType(value) == "bool") escapedValue = value;else escapedValue = value;
                }

                if (((typeof value === "undefined" ? "undefined" : _typeof(value)) != "object" || getType(value) == 'date') && getType(value) != 'bool') escapedValue = "'" + escapedValue + "'";
            ...
            */
            ...
            var template;
            if (!ignoreAddedProperties) template = addedHeaderFinal + '<div class="wrap"><ul class="custom-ul" {0} = " " {1} {2}>{3}</ul></div>'.format(this.type, addedClassesFinal, propertiesStr + " " + addedPropertyFinal, childrensStr) + addedFooterFinal;else template = addedHeaderFinal + '<{0} {1} {2}>{3}</{0}>'.format(this.type, addedClassesFinal, propertiesStr + " " + addedPropertyFinal, childrensStr) + addedFooterFinal;
    ...
    publicData.jqueryElementToElementInstance = function (jElement) {
        ...
        for (var propertie in elementInstance.properties) {
            ...
            if (typeof attrValue != 'undefined') {
                if (_typeof(elementInstance.properties[propertie]) != "object" && type != "bool") stringValue = jElement.attr(propertie).slice(1, -1);else stringValue = attrValue;

                if (type == "bool") elementInstance.properties[propertie] = parseBoolean(stringValue);else if (type == "array") elementInstance.properties[propertie] = JSON.parse(stringValue);else if (type == "date") elementInstance.properties[propertie] = new Date(stringValue);else if (type == "string") elementInstance.properties[propertie] = stringValue;
            }
