    ...
    publicData.ElementInstance = function(type) {
        ...
        object.getHTML = function(scope, ignoreAddedProperties) {
            ...
            /*if((scope.elementsListForLink.length - 1) %2 == 0)
                leftTooltip = "";*/
            var addedPropertyFinal = this.addedProperty + ' dnd-placeholder-body = "' + this.type + '" dnd-dragover="$root.dragoverCallback(event, index, external, type, $root.this)" dnd-drop="$root.dropCallback(event, index, item, external, type, $root.this)" dnd-list="$root.this.childrens"';
            var addedHeaderFinal = '<li  class="custom-grid-li"' + 'tooltip-placement="{1}" tooltip-trigger="mouseenter" uib-tooltip="{0}"'.format(this.type, leftTooltip) + 'class = "render-element" dnd-dragstart = "$root.dragStart($root.this)" dnd-draggable="$root.this" dnd-effect-allowed="move" dnd-selected="$root.models.selected = $root.this">';
