...
angular.module('springChat.directives').directive('botContainer', function ($compile, $parse) {
    return {
        ...
        link: function link(scope, element, attr, ctrl) {
            ...
            var processBotParameters = function processBotParameters(strWithParams) {
            ...
            scope.$watch('content', function () {
                ...
                if (scope.chatRouteInterfaceScope != null || scope.chatRouteInterfaceScope != undefined) scope.nextDialogItemJS = new Function("param", scope.currentMessage.testCase)(scope.chatRouteInterfaceScope.botParameters); //next item calc
                else scope.nextDialogItemJS = -1;

                var prefix = "";
                var sufix = "";
                if (scope.chatRouteInterfaceScope != null || scope.chatRouteInterfaceScope != undefined) var body = processBotParameters(parsedData.body);else var body = parsedData.body;
                ...
                if (attr.disabled != null || attr.disabled != undefined) scope.disabled = attr.disabled;
            ...
            if (scope.chatRouteInterfaceScope != null || scope.chatRouteInterfaceScope != undefined) {
                if (scope.chatRouteInterfaceScope.botContainers.length > 0) scope.chatRouteInterfaceScope.botContainers[scope.chatRouteInterfaceScope.botContainers.length - 1].scope.disabled = true;
...
angular.module('springChat.directives').directive('botList', function ($compile, $parse) {
    return {
        ...
        link: {
            post: function post(scope, element, attr, ctrl) {
        ...
        link: {
            post: function post(scope, element, attr, ctrl) {
                ...
                scope.$watch('href', function () {
                    if (typeof scope.href != 'undefined' && scope.href.length > 0) scope.ispost = false;
                });
                scope.itemvalue = body;
                if (usePost && typeof attr.linkindex !== 'undefined' && attr.linkindex.length > 0) {
                    ...
                    if (message.category != null) dataObject.category = message.category.id;
    ...
    return {
        ...
        link: function link(scope, element, attr, ctrl) {
            ...
            scope.submitBot = function (event) {
                ...
                if (scope.mainScope.currentRoom != undefined) urlPrefix = urlPrefix.format(scope.mainScope.currentRoom.roomId, scope.mainScope.currentMessage.idObject.id);else urlPrefix = urlPrefix.format(0, scope.mainScope.currentMessage.idObject.id); //send from quize
                ...
                $.ajax({
                    ...
                    success: function success(data) {
                        if (data == "quize save") alert("Ваша відповідь прийнята. Дякуємо за потрачений час"); // show response from the php script.
                    },
            ...
            scope.$watch('inputValue', function () {
                ...
                if (scope.keyPressProcessed == false) {
                    ...
                    for (var i = 0; i < scope.listData.length; i++) {
                        ...
                        if (size == 1) {
                            if (listElement[0] == scope.inputValue) result.push(listElement);
                        } else if (listElement.substring(0, scope.inputValue.length) == scope.inputValue) result.push(listElement);
                    }
...
function botrating() {
    return {
        controller: 'ChatViewItemController',
        restrict: 'EA',
        template: '<ul class="star-rating" ng-class="{readonly: readonly}">' + '<p>{{text}}</p>' + '  <li ng-repeat="star in stars" class="star" ng-class="{filled: star.filled}" ng-style="star.filled && {\'color\' : starcolor}" ng-click="toggle($index)">' + '    <i class="fa fa-star"></i>' + // or &#9733
        '  </li>' + '</ul>',
        scope: {
            itemvalue: '<',
        ...
        link: function link(scope, element, attributes) {
            ...
            if (typeof attributes.onratingselect == 'undefined' || attributes.onratingselect == "angular.noop") attributes.onratingselect = function () {};
...
function botcalendar($compile, $parse) {
    return {
        controller: 'ChatViewItemController',
        template: '<span class="input-group">' + '<input type="text" class="form-control" uib-datepicker-popup="{{format}}" ng-model="itemvalue" is-open="popup1.opened" datepicker-options="dateOptions" ng-required="true" close-text="Close" />' + '<span class="input-group-btn">' + '<button type="button" class="btn btn-default" ng-click="open1()"><i class="glyphicon glyphicon-calendar"></i></button>' + '</span>' + '</span>',
        scope: {
            itemvalue: '<', //date
            name: '='
        },
