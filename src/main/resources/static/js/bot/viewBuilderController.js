...
springChatControllers.controller('ChatViewBuilderController', ['$routeParams', '$rootScope', '$scope', '$window', '$uibModal', '$http', '$location', '$interval', '$cookies', '$timeout', 'toaster', 'ChatSocket', '$cookieStore', 'Scopes', '$q', '$controller', function ($routeParams, $rootScope, $scope, $window, $uibModal, $http, $location, $interval, $cookies, $timeout, toaster, chatSocket, $cookieStore, Scopes, $q, $controller) {
    ...
    var botDialogItemClean = function botDialogItemClean() {
        return {
            "body": "",
            "category": {
                "id": 1,
    ...
    $scope.updateLoadButton = function () {
        if ($scope.DialogItemInputMode === "auto" && $scope.selectedBotDialogItemForModal != null) {
            $scope.isLoadButtonDisabled = false;
        } else $scope.isLoadButtonDisabled = true;
        if ($scope.DialogItemInputMode === "manual") {
            if ($scope.selectedBotDialogItemForModalIdManually != null) $scope.isLoadButtonDisabled = false;else $scope.isLoadButtonDisabled = true;
        }
    };
    ...
    $scope.isColor = function (str) {
        if (str == "textcolor") return true;else false;
    };
    ...
    $scope.createBotDialogItem = function (description, categoryId) {
        ...
        $http.post(requestUrl, $scope.newDialogItem).success(function (data, status, headers, config) {
            //
    ...
    $scope.loadBotDialogItem = function () {
        var id;
        if ($scope.DialogItemInputMode == 'auto' && $scope.selectedBotDialogItemForModal != null) id = $scope.selectedBotDialogItemForModal.idObject.id;else if ($scope.DialogItemInputMode == 'manual') id = $scope.selectedBotDialogItemForModalIdManually;
        var botDialogItem = {};
        var requestUrl = serverPrefix + "/bot_operations/get_bot_dialog_item/{0}".format(id);

        $http.get(requestUrl, {}).success(loadView).error(function (data, status, headers, config) {});
    };
    ...
    $scope.initDialogItemModalLoader = function () {
        ...
        $scope.$watch('selectedBotDialogItemForModal', function (newVal, oldVal) {
            if (newVal != null && typeof newVal != 'undefined' && newVal.idObject != null && typeof newVal.idObject != 'undefined') $scope.selectedBotDialogItemForModalChanged(newVal.idObject.id);else $scope.selectedBotDialogItemForModalChanged("");
        });
    ...
    var loadAllCategories = function loadAllCategories() {
    ...
    var loadCategoriesIdsByName = function loadCategoriesIdsByName(name) {
    ...
    var loadDialogItemsIdsByDescription = function loadDialogItemsIdsByDescription(category, description) {
    ...
    $scope.reloadCategoriesByName = function (name) {
        loadCategoriesIdsByName(name).success(function () {
            if ($scope.botCategoriesIdsForModal.length > 0) $scope.selectedBotCategoryForModal = $scope.botCategoriesIdsForModal[0];
        });
    };
    $scope.reloadDialogItemsByDescription = function (category, description) {
        loadDialogItemsIdsByDescription(category, description).success(function () {
            if ($scope.botDialogItemsIdsForModal.length > 0) $scope.selectedBotDialogItemForModal = $scope.botDialogItemsIdsForModal[0];
        });
    };
    ...
    $scope.loadDialogItemsByDescription = function (categoryId, description) {
        ...
        var requestUrl = serverPrefix + "/bot_operations/get_bot_dialog_items_descriptions_having_string_first5/" + categoryId + "/" + encodeURIComponent(description);
    ...
    $scope.saveBotDialogItem = function () {
        ...
        $http.post(requestUrl, object).success(function (data, status, headers, config) {
            $scope.tabsChanges[$scope.viewTabs.length - 1] = false;
    ...
    $scope.setBotDialogItemAsDefault = function (object) {
        ...
        $http.get(requestUrl, object).success(function (data, status, headers, config) {}).error(function (data, status, headers, config) {
    ...
    $scope.onExit = function () {
        ...
        if (str.length * 2 > 4000) return "All your changes can not be stored in temporary storage. Some changes will be lost !!!?";
    };
    ...
    function loadFromCookise() {
        ...
        var obj;
        if (cookies != null && cookies != undefined) obj = JSON.parse($cookies.get('save_object'));
    ...
    $scope.tryCloseTabDialogClose = function (b_closeTab) {
        ...
        if (b_closeTab == true) {
            if ($scope.tryCloseTabDialog.index == null || $scope.tryCloseTabDialog.index == undefined) closeTab($scope.activeViewTab - 1);else closeTab($scope.tryCloseTabDialog.index);
        }
        $scope.tryCloseTabDialog = null;
    };
