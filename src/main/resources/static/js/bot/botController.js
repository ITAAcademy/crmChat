...
springChatControllers.controller('ChatBotController', ['$routeParams', '$rootScope', '$scope', '$http', '$location', '$interval', '$cookies', '$timeout', 'toaster', 'ChatSocket', '$cookieStore', 'Scopes', '$q', '$controller', function ($routeParams, $rootScope, $scope, $http, $location, $interval, $cookies, $timeout, toaster, chatSocket, $cookieStore, Scopes, $q, $controller) {
    ...
    if (chatRouteInterfaceScope != null || chatRouteInterfaceScope != undefined) chatRouteInterfaceScope.$watch('participants', function () {
        if (chatRouteInterfaceScope.participants.length > 2) {
            ...
            toaster.clear();
        } else chatControllerScope.currentRoom.active = false;
    });

    var askIsFreeTenant;

    $scope.giveTenant = function () {
        $http.post(serverPrefix + "/bot_operations/close/{0}".format(chatControllerScope.currentRoom.roomId)).success(function (data, status, headers, config) {
            {
    ...
    $scope.getParamsInJSON = function (formData) {
        //concat(arrayB);
        if ($scope.itemvalue != undefined) formData[$scope.name] = JSON.stringify($scope.itemvalue) || "";
