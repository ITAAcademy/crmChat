...
springChatServices.factory('ChatSocket', ['$rootScope', function ($rootScope) {
    ...
    var wrappedSocket = {
        ...
        connect: function connect(successCallback, errorCallback) {
        ...
        subscribe: function subscribe(destination, callback) {
        ...
        send: function send(destination, headers, object) {
...
springChatServices.service('AskWindow', ['$rootScope', 'ngDialog', '$timeout', '$http', '$injector', function ($rootScope, ngDialog, $timeout, $http, $injector) {
    ...
    this.showAskWindow = function () {
        ...
        hideAskTenantToTakeConsultation_tenantNotRespond = $timeout(function () {
            if (tenantInviteDialog != null) tenantInviteDialog.close();
    ...
    $rootScope.answerToTakeConsultation = function (value) {
        ...
        if (value) {
            ...
            $http.post(serverPrefix + noLink).success(function (data, status, headers, config) {}).error(function (data, status, headers, config) {
                alert("error : " + status);
            });
        }
    };
