...
var chatController = springChatControllers.controller('ChatController', ['$sce', 'ngDialog', '$q', '$rootScope', '$scope', '$http', '$route', '$location', '$interval', '$cookies', '$timeout', 'toaster', '$cookieStore', 'RoomsFactory', 'UserFactory', 'ChannelFactory', function ($sce, ngDialog, $q, $rootScope, $scope, $http, $route, $location, $interval, $cookies, $timeout, toaster, $cookieStore, RoomsFactory, UserFactory, ChannelFactory) {
    ...
    $rootScope.needShowDayDivided = function (curr, prev) {
        if (checkIfToday(curr) || checkIfYesterday(curr)) return true;
    ...
    *
    ...
    $scope.blocksItems = [[new BlockItem("", "Людмила Журавская")], [new BlockItem("", "Василій Пупкін", true), new BlockItem("", "Микола Петряк")]];
    ...
    var toggleAskForDeleteMe = function toggleAskForDeleteMe(room) {

        // if (event != undefined && event != null)
        //    event.stopPropagation();

        if (room != undefined && room != null) $rootScope.askForDeleteMe = { "room": room, isAuthor: UserFactory.getChatUserId() == room.roomAuthorId };
        ngDialog.open({
    ...
    $scope.deleteMeFromRoom = function () {
        var showERR = function showERR() {
            $rootScope.askForDeleteMe.error = "Не вдалося видалити Вас з розмови!!!";
        };
        $http.post(serverPrefix + "/chat/rooms/{0}/remove".format($rootScope.askForDeleteMe.room.roomId)).success(function (data, status, headers, config) {
            if (data == true) ngDialog.closeAll();else showERR();
    ...
    $rootScope.goToAuthorize = function (func) {
        ...
        } else $rootScope.redirectzToAuthorizePage();
    ...
    $rootScope.getUserSearchIndetify = function (item, searchInputValue) {
        ...
        if (item.secondName != null) name += item.secondName;

        if (searchInputValue != null && item.email != null && item.email.indexOf(searchInputValue) != -1 || name == null || name == ' ') return item.email;else return name;
    };
    ...
    $scope.changeUser = function (chatUserId, chatUserNickName) {
        ...
        if ($rootScope.socketSupport) ChannelFactory.initForWS(true);else ChannelFactory.reInitForLP();
    ...
    $rootScope.loadOtherMessages = function () {
        if ($rootScope.message_busy || RoomsFactory.getCurrentRoom() == null) return;
        ...
        if ($scope.messageSearchEnabled) payload['searchQuery'] = $scope.messageSearchQuery.value;
    ...
    $rootScope.checkPrivateRelations = function (room, user) {
        ...
        if (room.type == 1 && room.privateUserIds != undefined) {
            ...
            if (room.privateUserIds[0] == user.chatUserId || room.privateUserIds[1] == user.chatUserId) return true;
        }
        return false;
    };
    ...
    function newMessageMapEventHandler(event, messagesMap) {
        ...
        for (var roomIndex = 0; roomIndex < RoomsFactory.getRooms().length; roomIndex++) {
            ...
            if (newMessageInThisRoom != -1) {
                ...
                if (RoomsFactory.getCurrentRoom() == undefined || RoomsFactory.getCurrentRoom().roomId != room.roomId) {
                    ...
                    if ($scope.soundEnable) new Audio('data/new_mess.mp3').play();
        ...
        if (RoomsFactory.getCurrentRoom() != undefined) if (RoomsFactory.getCurrentRoom().roomId == "" && $rootScope.roomForUpdate.keys().next() != undefined) {
            if (updateDialogListTimeout != null) updateDialogListTimeout.cancel();
    ...
    $scope.getKeys = function (obj) {
        ...
        for (var i = 0; i < keys.length; i++) {
            var value = undefined;
            if (obj[keys[i]] != undefined) value = obj[keys[i]];
    ...
    $scope.updateMessagesSearch = function () {
        if (updateMessagesSearchTimeout != undefined) $timeout.cancel(updateMessagesSearchTimeout);
    ...
    $scope.focusMessagesSearchChange = function () {
        if ($scope.messageSearchQuery.value == '') $scope.disableMessagesSearch(false);
    };
    ...
    $scope.disableMessagesSearch = function (reloadMessages) {
        ...
        $scope.messageSearchEnabled = false;
        var objDiv = document.getElementById("messagesScroll");
        if (objDiv != null) objDiv.scrollTop = 99999999999; //objDiv.scrollHeight;
    };
    ...
    function saveScrollBottom() {
        ...
        savedDistanceToBottom = typeof messagesScroll === "undefined" || typeof messagesScroll[0] === "undefined" ? 0 : messagesScroll[0].scrollHeight - messagesScroll.scrollTop() - messagesScroll[0].clientHeight;
    }
    ...
    function getScrollTopToPreserveScroll(futureHeight) {
        ...
        var scrollTop = scrollHeight - savedDistanceToBottom - messagesScroll[0].clientHeight;
    ...
    function messageAreaResizer(e) {
        //  $('#messagesScroll').stop();
        if (delay != null) $timeout.cancel(delay);
    ...
    $scope.$$postDigest(function () {
        ...
        $scope.canLeaveCurrentRoom = function () {
            if (RoomsFactory.getCurrentRoom() == null || RoomsFactory.getCurrentRoom().type == 1) return false;
            return true;
        };
        ...
        $scope.confirmToHelp = function (roomId) {
            $http.post(serverPrefix + "/bot_operations/triner/confirmToHelp/" + roomId, {}).success(function (data, status, headers, config) {
