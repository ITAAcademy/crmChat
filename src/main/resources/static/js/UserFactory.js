...
springChatServices.factory('UserFactory', ['$timeout', '$rootScope', '$location', '$http', 'toaster', '$injector', 'ChannelFactory', 'ChatSocket', 'AskWindow', function ($timeout, $rootScope, $location, $http, toaster, $injector, ChannelFactory, chatSocket, AskWindow) {
    ...
    var getRoomsRequiredTrainers = function getRoomsRequiredTrainers() {
        return roomsRequiredTrainers;
    };
    var getRoomsRequiredTrainersLength = function getRoomsRequiredTrainersLength() {
        return roomsRequiredTrainers == null ? 0 : Object.keys(roomsRequiredTrainers).length;
    };
    var isMessageSended = function isMessageSended() {
        return messageSended;
    };
    var setMessageSended = function setMessageSended(val) {
        messageSended = val;
    };

    var isTenantFree = true;
    var setTenantFree = function setTenantFree() {
        isTenantFree = true;
    };
    var setTenantBusy = function setTenantBusy() {
        isTenantFree = false;
    };
    var getTenantIsFree = function getTenantIsFree() {
        return isTenantFree;
    };

    var checkRole = function checkRole() {
        if (chatUserRole & 256) return true;
        return false;
    };
    var isUserOnline = function isUserOnline(chatUserId) {
        return onlineUsersIds.indexOf(chatUserId) == -1 ? false : true;
    };
    var setOnlineUsersIds = function setOnlineUsersIds(ids) {
        onlineUsersIds = ids;
    };
    ...
    var initStompClient = function initStompClient() {

        function reInitForLP() {
            $http.post(serverPrefix + "/chat/login/" + getChatUserId(), { message: 'true' }).success(function (data, status, headers, config) {
    ...
    function initForWS(reInit) {
        chatSocket.subscribe("/app/chat.login/{0}".format(getChatUserId()), function (message) {
            ...
            if (reInit == false) $timeout(function () {
                ...
                chatSocket.subscribe("/topic/{0}/must/get.room.num/chat.message".format(getChatUserId()), function (message) {
                    // event update
                ...
                chatSocket.subscribe("/topic/users/{0}/status".format(getChatUserId()), function (message) {
                    ...
                    switch (operationStatus.type) {
                        ...
                        case Operations.send_message_to_user:
                            messageSended = true;
                            if (!operationStatus.success) toaster.pop("error", "Error", message.body);
                            break;
                        case Operations.add_user_to_room:
                            userAddedToRoom = true;
                            if (!operationStatus.success) toaster.pop("error", "Error", "user wasn't added to room");
                            break;
                        case Operations.add_room:
                            roomAdded = true;
                            if (!operationStatus.success) toaster.pop("error", "Error", "room wasn't added");
                            break;
        ...
        chatSocket.subscribe("/topic/chat/rooms/user.{0}".format(getChatUserId()), function (message) {
            // event update
    ...
    function subscribeInfoUpdateLP() {
        ...
        $http.post(serverPrefix + "/chat/global/lp/info").success(function (data, status, headers, config) {
    ...
    function login(mess_obj) {
        ...
        if (mess_obj.nextWindow == 0) {
            ...
            if ($location.path() == "/") ChannelFactory.changeLocation("/chatrooms");
        } else {
            $rootScope.authorize = false;
    ...
    var initIsUserTenant = function initIsUserTenant() {
        if (isUserTenantInited == false) {
            $http.post(serverPrefix + "/bot_operations/tenant/did_am_busy_tenant").success(function (data, status, headers, config) {

                isTenant = data[0];
                if (data[0]) isTenantFree = !data[1];
    ...
    var setChatUserId = function setChatUserId(id) {
        chatUserId = id;
    };
    var getChatUserId = function getChatUserId() {
        return chatUserId;
    };
    var setRealChatUserId = function setRealChatUserId(id) {
        realChatUserId = id;
    };

    var participantsSort = function participantsSort(participant) {
    ...
    return {
        ...
        login: login,
        getChatUserNickname: function getChatUserNickname() {
            return chatUserNickname;
        },
        getChatuserAvatar: function getChatuserAvatar() {
            return chatUserAvatar;
        },
        getTenantsList: function getTenantsList() {
            return tenants;
        },
        getStudentTrainerList: function getStudentTrainerList() {
