...
var imageDrop = function imageDrop($parse, $document) {
    return {
        restrict: "A",
...
var autoGrow = function autoGrow() {
...
var dirFunc = function dirFunc($compile, $parse) {
    return {
        restrict: 'E',
        link: function link(scope, element, attr) {
            scope.$watch(attr.content, function () {
                ...
                if (typeof attr.callback != 'undefined') {
                    ...
                    if (typeof callBackFunction != 'undefined') callBackFunction(element);
                }
...
var modaleToggle = function modaleToggle($compile, $parse, $rootScope) {
    return {
        restrict: 'EA',
        scope: {
        ...
        link: function link(scope, element, attr) {
            ...
            } else ignoredList.push(scope.ignoreId);
            if (scope.callback != null) {
                $(window).click(function (e) {
                    ...
                    for (var i = 0; i < ignoredList.length; i++) {
                        ...
                        if (ignoredElement == null) continue;
                    ...
                    if (e.target === element[0] || element[0].contains(e.target) || toggle && ignore) {
                        ...
                        scope.$apply(function () {
                            ...
                            if (res == undefined) toggle = !toggle;else toggle = res;
                        });
                    }
...
function starRating() {
    return {
        ...
        link: function link(scope, element, attributes) {
            ...
            if (attributes.onratingselect == "angular.noop") attributes.onratingselect = function () {};
...
var tenantsBlock = function tenantsBlock($http, mySettings, UserFactory, RoomsFactory) {
    return {
        restrict: 'EA',
        scope: {},
...
function initFolded(scope, element) {
    ...
    scope.toggleFolded = function (event) {
        if (event != undefined && $(event.target).hasClass("block_controll")) return;
        scope.folded = !scope.folded;
        scope.scroll.overflowy = !scope.folded;
        if (scope.folded) scope.scroll.scrollTop(scope.scroll.scrollTop());
    };
    ...
    return {
        ...
        link: function link(scope, element, attributes) {
            ...
            scope.toggleNewUser = function () {
                ...
                if (toggleNewUser) scope.$root.$emit('rootScope:roomsBlockModeChange', 2);else scope.$root.$emit('rootScope:roomsBlockModeChange', 1);
...
angular.module('springChat.directives').directive('messageInput', ['$http', 'RoomsFactory', 'ChatSocket', '$timeout', 'UserFactory', 'ChannelFactory', messageInput]);
    ...
    return {
        ...
        link: function link($scope, element, attributes) {
            $scope.sendMessage = function (message, attaches, clearMessageInput) {
                ...
                var textOfMessage;
                if (typeof message === "undefined") textOfMessage = $scope.newMessage;else textOfMessage = message;
                ...
                if (ChannelFactory.isSocketSupport() == true) {
                    ...
                    $http.post(serverPrefix + "/{0}/chat/message".format(RoomsFactory.getCurrentRoom().roomId), msgObj).success(function (data, status, headers, config) {
                ...
                if (message === undefined) $scope.newMessage = '';
            };
            ...
            $scope.sendMessageAndFiles = function () {
                ...
                if (files != null && files.length > 0) {
                    uploadXhr(files, "upload_file/" + RoomsFactory.getCurrentRoom().roomId, function successCallback(data) {
...
var roomsBlockFilter = function roomsBlockFilter(RoomsFactory) {
    return function (fields, state) {
        if (fields) {
            // added check for safe code
            var arrayFields = [];
...
roomsBlockLinkFunction = function roomsBlockLinkFunction($scope, element, attributes, $http, RoomsFactory, ChannelFactory, UserFactory) {
    ...
    $scope.clickToRoomEvent = function (room) {
        ...
        switch ($scope.mode) {
            case 1:
                ...
                break;
            case 2:
                if (room.privateUserIds[0] == UserFactory.getChatUserId()) addNewUser(room.privateUserIds[1]);
                if (room.privateUserIds[1] == UserFactory.getChatUserId()) addNewUser(room.privateUserIds[0]);
                $scope.mode = 1;
                break;
        }
        $scope.$root.hideMenu();
    ...
    $scope.canBeRemoved = function (room) {
        if (room.type == 1) return false;
        return true;
    };
    $scope.showLastContacts = function () {
        if ($scope.mode == 2) return;
    ...
    $scope.returnAvatar = function (room) {
        ...
        if (room.avatars.length > 1) return room.avatars[1];
    ...
    $scope.addForCreatRoom = function (room) {
        ...
        if (room.type == 1 && room.privateUserIds != undefined) {
            if (room.privateUserIds[0] == UserFactory.getChatUserId()) userListForAddedToNewRoom.push(room.privateUserIds[1]);
            if (room.privateUserIds[1] == UserFactory.getChatUserId()) userListForAddedToNewRoom.push(room.privateUserIds[0]);
        }
    };
...
function fileMiniature($http, RoomsFactory, ChannelFactory, $parse) {
    return {
        ...
        link: function link($scope, element, attributes) {
            //TODO filesMiniature
            var supportedExtensions = ['aac', 'ai',, 'aiff', 'asp', 'avi', 'bmp', 'c', 'cpp', 'css', 'dat', 'dmg', 'doc', 'docx', 'dot', 'dotx', 'dwg', 'dxf', 'eps', 'exe', 'flv', 'gif', 'h', 'html', 'ics', 'iso', 'java', 'jpg', 'js', 'key', 'less', 'm4v', 'mid', 'mov', 'mp3', 'mp4', 'mpg', 'odp', 'ods', 'odt', 'otp', 'ots', 'ott', 'pdf', 'php', 'png', 'pps', 'ppt', 'psd', 'py', 'qt', 'rar', 'rb', 'rtf', 'sass', 'scss', 'sql', 'tga', 'tgz', 'tiff', 'txt', 'wav', 'xls', 'xlsx', 'xml', 'yml', 'zip'];
            var getFileExtensionByName = function getFileExtensionByName(name) {
                return name.split('.').pop();
            };
            var isExtensionSupported = function isExtensionSupported(extension) {
                if (supportedExtensions.indexOf(extension) != -1) return true;else return false;
            };
            var getImageByExtension = function getImageByExtension(ext) {
                var urlTemplate = "images/svg-file-icons/{0}.svg";
                if (isExtensionSupported(ext)) return urlTemplate.format(ext);else return urlTemplate.format('nopreview');
            };
            ...
            if (!nameOnly) $scope.link = link;
            $scope.imageSrc = getImageByExtension(extension);
        }

    };
};
var compilable = function compilable($compile, $parse) {
    return {
        restrict: 'E',
        link: function link(scope, element, attr) {
            scope.$watch(attr.content, function () {
...
var ngDraggable = function ngDraggable($document) {
    return {
        restrict: 'A',
        scope: {
        ...
        link: function link(scope, elem, attr) {
            ...
            elem.on('mousedown', function (e) {
                ...
                if ($(dragElement).hasClass("drag-disable") || e.target != elem[0]) return true;
    ...
    return {
        ...
        link: function link(scope, element, attrs) {
            ...
            element.bind('click', function () {
                ...
                if (scope.ngChange != undefined) scope.ngChange();
                scope.$apply();
            });
        }
