...
var checkIfToday = function checkIfToday(inputDateLong) {
    var inputDate = new Date(inputDateLong);
    var todaysDate = new Date();
    return inputDate.setHours(0, 0, 0, 0) == todaysDate.setHours(0, 0, 0, 0);
};
var checkIfYesterday = function checkIfYesterday(inputDateLong) {
    var inputDate = new Date(inputDateLong);
...
var isSameDay = function isSameDay(currentdate, previus) {
    if (currentdate == undefined && previus == undefined) return true;
...
var getNameFromRandomizedUrl = function getNameFromRandomizedUrl(url) {
...
var firstLetter = function firstLetter(name) {
    if (undefined != name) return name.toUpperCase().charAt(0);
};

var formatDateWithLast = function formatDateWithLast(date, short, withoutTime, max_time) {
    if (short == undefined) short = false;
    if (withoutTime == undefined) withoutTime = false;
    if (max_time == undefined) max_time = 60;
    ...
    if (dateObj == null || dateObj == undefined || isNaN(dateObj)) return "";

    var delta = new Date().getTime() - dateObj.getTime();
    if (delta > 60000 * max_time) return formatDate(date, short, withoutTime);else if (Math.round(delta / 60000) == 0) return "щойно";

    var minutesStr = Math.round(delta / 60000);
    if (short) {
        if (minutesStr < 60) return minutesStr + minutesNameShort[globalConfig.lang];else if (Math.ceil(minutesStr / 60) < 24) return Math.ceil(minutesStr / 60) + hoursNameShort[globalConfig.lang];else return Math.ceil(minutesStr / (60 * 24)) + daysNameShort[globalConfig.lang];
    } else {
        if (minutesStr > 1) {
            if (minutesStr < 60) return minutesStr + minutesName[globalConfig.lang] + endName[globalConfig.lang];else {
                var hours = Math.ceil(minutesStr / 60);
                if (hours < 24) {
                    if (hours > 1) return hours + hoursName[globalConfig.lang] + endName[globalConfig.lang];else return hours + hourName[globalConfig.lang] + endName[globalConfig.lang];
                } else {
                    var days = Math.ceil(hours / 24);
                    if (days > 1) return days + daysName[globalConfig.lang] + endName[globalConfig.lang];else return days + dayName[globalConfig.lang] + endName[globalConfig.lang];
                }
            }
        } else return minutesStr + minuteName[globalConfig.lang] + endName[globalConfig.lang];
    }
};

var differenceInSecondsBetweenDates = function differenceInSecondsBetweenDates(t1, t2) {
    var dif = t1.getTime() - t2.getTime();
    var Seconds_from_T1_to_T2 = dif / 1000;
    var Seconds_Between_Dates = Math.abs(Seconds_from_T1_to_T2);
    return Seconds_Between_Dates;
};
var formatDate = function formatDate(date, short, withoutTime) {
    // need translate and move to global to config map
    if (short == undefined) short = false;

    var monthNames = {};
    monthNames['ua'] = ["Січеня", "Лютого", "Березеня ", "Квітня", "Травня ", "Червня ", "Липня", "Серпня", "Вересеня", "Жовтеня", "Листопада", "Груденя"];
    monthNames['en'] = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    monthNames['ru'] = ["Января", "Февраля", "Марта", "Апреля", "Мая", "Июня", "Июля", "Августа", "Сентября", "Октября", "Ноября", "Декабря"];
    var dateObj = new Date(date);
    ...
    if (short) {
        if (monthIndex + 1 < 10) return day + ".0" + (monthIndex + 1);else return day + "." + (monthIndex + 1);
    }
    var res = day + " " + monthNames[globalConfig.lang][monthIndex];
    if (!withoutTime) res += " " + dateObj.getHours() + ":" + minutes;
    return res;
};
...
var curentDateInJavaFromat = function curentDateInJavaFromat() {
    var currentdate = new Date();
    ...
    var datetime = currentdate.getFullYear() + "-" + mouth + "-" + day + " " + currentdate.getHours() + ":" + currentdate.getMinutes() + ":" + currentdate.getSeconds() + ".0";
...
function upload($http, files, urlpath) {
    ...
    return $http.post(urlpath, formData, {
        transformRequest: function transformRequest(data, headersGetterFunction) {
            return data;
        },
